...
parser.add_argument(
    '--param', type=bool, default=False, 
    help='if you want to use model that allows for more params'
)
...
train_gen = DataGenerator_Gaussian_Labels_Image_Only(
...
with strategy.scope():
    if args.param:
        dropout = experiment_dict[experiment_name]['dropout']
        image_features = experiment_dict[experiment_name]['image_features']

        network = AD_classifier_plus_tuning_img_only(
            width=data_dim[0], 
            height=data_dim[1], 
            depth=data_dim[2],
            dropout = dropout, 
        ...
        network = AD_classifier(width=data_dim[0], height=data_dim[1], depth=data_dim[2]) 
        network.compile(
            optimizer = Adam(learning_rate=lr_schedule),
            loss=tf.keras.losses.binary_crossentropy, 
            metrics=['binary_accuracy']
...
checkpoint_cb = keras.callbacks.ModelCheckpoint(
    '../data/saved_models_v2/{}.h5'.format(experiment_name+'_gaussian_labeled_img_only'), save_best_only=True
)
        
if args.load:
    saved_model = '../data/saved_models_v2/{}.h5'.format(experiment_name+'_gaussian_labeled_img_only')
    network.load_weights(saved_model)

print('#'*20)
print('TRAINING ONLY USING IMAGES AND GAUSSIAN LABELS; NO COVARIATE DATA INCLUDED')
print('#'*20)
...
np.save('../data/saved_models_v2/{}_history.npy'.format(experiment_name+'_gaussian_labeled_img_only'), training_history.history)