...
parser.add_argument('--alt', type=bool, default=False, help='if you want to use other dropout model')
parser.add_argument(
...
with strategy.scope():
    if args.param:
        ...
        network = AD_classifier_plus_alt_dropout(width=data_dim[0], height=data_dim[1], depth=data_dim[2]) 
        network.compile(
            optimizer = Adam(learning_rate=lr_schedule),
            loss=tf.keras.losses.binary_crossentropy, 
            metrics=['binary_accuracy']
...
output_str = experiment_name+'_gaussian_labels'
...
checkpoint_cb = keras.callbacks.ModelCheckpoint(
    '../data/saved_models_v2/{}.h5'.format(output_str), save_best_only=True
)
        
if args.load:
    saved_model = '../data/saved_models_v2/{}.h5'.format(output_str)
    network.load_weights(saved_model)

print('#'*30)
print('TRAINING USING GAUSSIAN LABELS')
print('#'*30)

if args.alt:
    print('USING ALT DROPOUT LAYER PLACEMENT')
...
np.save('../data/saved_models_v2/{}_history.npy'.format(output_str), training_history.history)